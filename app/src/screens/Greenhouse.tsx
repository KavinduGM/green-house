...
import { Camera, Plus, Check, Trash2, Sparkles, ImageDown } from 'lucide-react';
...
export default function Greenhouse() {
  ...
  const addBag = async () => {
    ...
    await api.post('/api/grow-bags', { label: newLabel.trim(), x: 0.2 + Math.random() * 0.6, y: 0.2 + Math.random() * 0.6 });
  ...
  return (
    <div className="px-4 pt-4 space-y-4">
      <header><h1 className="text-xl font-bold text-leaf-800">Greenhouse layout</h1>
        <p className="text-sm text-gray-400">Snap your drawing — AI lays out the grow bags.</p></header>
      ...
      <Card>
        ...
        {bags.length === 0 ? (
          <Empty title="No grow bags yet" hint="Scan a hand-drawn diagram or add bags manually." />
        ) : (
          <div className="relative w-full rounded-xl bg-leaf-50/60 border border-leaf-100 overflow-hidden" style={{ aspectRatio: '4 / 3' }}>
            {bags.map((b) => {
              const pl = bagPlanting(b.id);
              return (
                <div key={b.id} className="absolute -translate-x-1/2 -translate-y-1/2 flex flex-col items-center"
                  style={{ left: `${b.x * 100}%`, top: `${b.y * 100}%` }}>
                  <div className={`w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold shadow ${
                    pl ? 'bg-leaf-600 text-white' : 'bg-white text-gray-400 border border-gray-200'}`}>
                    {b.label}
                  </div>
                  {pl && <span className="text-[9px] mt-0.5 text-leaf-700 font-medium max-w-[60px] truncate">{pl.sinhala}</span>}
                </div>
              );
            })}
          </div>
      ...
      <Modal open={!!preview} onClose={() => setPreview(null)} title="AI-detected layout">
        {preview && (
          <div className="space-y-3">
            ...
            <p className="text-xs text-gray-400">Applying replaces your current bag positions.</p>
          </div>
        )}
      </Modal>
